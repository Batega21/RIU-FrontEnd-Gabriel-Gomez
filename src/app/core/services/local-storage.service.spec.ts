...
describe('LocalStorageService', () => {
  ...
  it('should set an item in localStorage', () => {
    ...
    service.addHeroesInStorage(value);
    const storedValue = service.getStoredHeroesByKey(key);
  ...
  it('should get an Heroes from localStorage', () => {
    ...
    const paginatedHeroes = service.getLocalHeroesPaginated(Pagination.DEFAULT_PAGE, Pagination.DEFAULT_LIMIT);
  ...
  it('should return null if the item does not exist in localStorage', () => {
    const key = 'testKey';
    const retrievedValue = service.getStoredHeroesByKey(key);
  ...
  it('should remove a Hero from the local storage', () => {
    ...
    service.addHeroesInStorage(value);
    service.deleteHeroFromStorage(heroId);

    const updatedHeroes = service.getStoredHeroesByKey(key);
  ...
  it('should not throw an error when removing a non-existent item from localStorage', () => {
    ...
    expect(() => service.deleteHeroFromStorage(heroId)).not.toThrow();
  ...
  it('should add a new hero to storage when heroes already exist', () => {
    ...
    service.addHeroesInStorage(initial);
    const newHero = { id: 999, name: 'Extra Hero' };
    service.addHeroToStorage(newHero as any, initial.heroesCount);
    const stored = service.getStoredHeroesByKey('heroes');
  ...
  it('should not add a hero if hero with same id already exists', () => {
    ...
    service.addHeroesInStorage(initial);
    spyOn(console, 'warn');
    service.addHeroToStorage(hero as any, initial.heroesCount);
    const stored = service.getStoredHeroesByKey('heroes');
  ...
  it('should add a new hero to storage when no heroes exist', () => {
    ...
    service.addHeroToStorage(newHero, HEROES_MOCK.length);
    service.addHeroesInStorage({ heroes: [newHero], heroesCount: 1 });
    const stored = service.getStoredHeroesByKey('heroes');
  ...
  it('should return null from getLocalHeroesPaginated if no heroes in storage', () => {
    localStorage.clear();
    const result = service.getLocalHeroesPaginated(1, 10);
    expect(result).toBeNull();
  });

  it('should return paginated heroes from getLocalHeroesPaginated', () => {
    const value = { heroes: HEROES_MOCK, heroesCount: HEROES_MOCK.length };
    service.addHeroesInStorage(value);
    const page = 2;
    const pageSize = 2;
    const expected = HEROES_MOCK.slice((page - 1) * pageSize, page * pageSize);
    const result = service.getLocalHeroesPaginated(page, pageSize);
  ...
  it('should log error and return null if getLocalHeroes is called with missing key', () => {
    spyOn(console, 'error');
    const result = service.getStoredHeroesByKey('missingKey');
    expect(result).toBeNull();
  ...
  it('should log warning if removeHeroesFromStorage is called when no heroes exist', () => {
    spyOn(console, 'warn');
    service.deleteHeroFromStorage(123);
